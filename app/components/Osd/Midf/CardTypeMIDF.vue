...
import type { MIDF_AboutTheWorkDataTarget } from '~/utils/AboutTheWorkData'
import type { CampiAggiuntiviDataTarget } from '~/utils/CampiAggiuntivi'
import type { MIDF_HeaderDataTarget } from '~/utils/HeaderData'
import type { MIDF_ManifestDataTarget } from '~/utils/ManifestData'
import type { MIDF_MetaDataTarget } from '~/utils/MetaData'
import type { MIDF_TitleDataTarget } from '~/utils/TitleData'
...
import { MIDF_AboutTheWorkData } from '~/utils/AboutTheWorkData'
import { GEN_CampiAggiuntiviData } from '~/utils/CampiAggiuntivi'
import { MIDF_HeaderData } from '~/utils/HeaderData'
import { MIDF_ManifestData } from '~/utils/ManifestData'
import { MIDF_MetaData } from '~/utils/MetaData'
import { MIDF_TitleData } from '~/utils/TitleData'
...
const titleData: MIDF_TitleDataTarget = reactive({ titolo: '', autore: '', titoloProprio: '' })
const headerData: MIDF_HeaderDataTarget = reactive({ tipologiaOggetto: '', soggetto: '', datazione: '', istitutoDiConservazione: '' })
const metaData: MIDF_MetaDataTarget = reactive({ titolo: '', autoreOriginale: '', soggetto: '', autore: '', ambitoCulturale: '', datazione: '', luogoRipresa: '', dataRipresa: '', indicazionidiColore: '', iscrizioni: '', tipologiaOggetto: '', funzionaOriginaria: '', tecnica_e_supporto: '', misure: '', analyticalTechnique: '', acquisizione: '', istitutoDiConservazione: '', mostre: '', bibliografia: '', restauri: '' })
const aboutTheWorkData: MIDF_AboutTheWorkDataTarget = reactive({ about: '' })
const manifestData: MIDF_ManifestDataTarget = reactive({ autore: '', anno: '', tecnica: '', luogo: '' })
const campiAggiuntiviData: CampiAggiuntiviDataTarget = reactive({ campoAggiuntivo: [{ label: '', value: '' }] })

const titleDataExtractor = new MIDF_TitleData(titleData)
const headerDataExtractor = new MIDF_HeaderData(headerData)
const aboutTheWorkExtractor = new MIDF_AboutTheWorkData(aboutTheWorkData)
const metaDataExtractor = new MIDF_MetaData(metaData)
const manifestExtractor = new MIDF_ManifestData(manifestData)
