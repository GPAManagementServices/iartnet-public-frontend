...
import type { F_AboutTheWorkDataTarget } from '~/utils/AboutTheWorkData'
import type { CampiAggiuntiviDataTarget } from '~/utils/CampiAggiuntivi'
import type { F_HeaderDataTarget } from '~/utils/HeaderData'
import type { F_ManifestDataTarget } from '~/utils/ManifestData'
import type { F_MetaDataTarget } from '~/utils/MetaData'
import type { F_TitleDataTarget } from '~/utils/TitleData'
...
import { F_AboutTheWorkData } from '~/utils/AboutTheWorkData'
import { GEN_CampiAggiuntiviData } from '~/utils/CampiAggiuntivi'
import { F_HeaderData } from '~/utils/HeaderData'
import { F_ManifestData } from '~/utils/ManifestData'
import { F_MetaData } from '~/utils/MetaData'
import { F_TitleData } from '~/utils/TitleData'
...
const titleData: F_TitleDataTarget = reactive({ autore: '', titolo: '', anno: '', editori: '' })
const headerData: F_HeaderDataTarget = reactive({ autoreOriginale: '', soggetto: '', datazione: '', funzioneOriginaria: '', tipologiaOggetto: '', istitutoDiConservazione: '' })
const metaData: F_MetaDataTarget = reactive({ titolo: '', autore: '', soggetto: '', datazione: '', iscrizioni: '', stemmi: '', tipologiaOggetto: '', funzionaOriginaria: '', tecnica_e_supporto: '', misure: '', luogoRipresa: '', dataRipresa: '', indicazionidiColore: '', formato: '', collocazione: '', bibliografia: '', bibliografia_AD: '' })
const aboutTheWorkData: F_AboutTheWorkDataTarget = reactive({ about: '' })
const manifestData: F_ManifestDataTarget = reactive({ autore: '', anno: '', tecnica: '', luogo: '' })
const campiAggiuntiviData: CampiAggiuntiviDataTarget = reactive({ campoAggiuntivo: [{ label: '', value: '' }] })

const titleDataExtractor = new F_TitleData(titleData)
const headerDataExtractor = new F_HeaderData(headerData)
const aboutTheWorkExtractor = new F_AboutTheWorkData(aboutTheWorkData)
const metaDataExtractor = new F_MetaData(metaData)
const manifestExtractor = new F_ManifestData(manifestData)
