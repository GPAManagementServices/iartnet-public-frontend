...
import type { S_HeaderDataTarget } from '~/utils/HeaderData'
import type { S_ManifestDataTarget } from '~/utils/ManifestData'
import type { S_MetaDataTarget } from '~/utils/MetaData'
import type { S_TitleDataTarget } from '~/utils/TitleData'
...
import { S_AboutTheWorkData } from '~/utils/AboutTheWorkData'
import { GEN_CampiAggiuntiviData } from '~/utils/CampiAggiuntivi'
import { S_HeaderData } from '~/utils/HeaderData'
import { S_ManifestData } from '~/utils/ManifestData'
import { S_MetaData } from '~/utils/MetaData'
import { S_TitleData } from '~/utils/TitleData'
...
const titleData: S_TitleDataTarget = reactive({ autore: '', titolo: '', anno: '', editori: '' })
const headerData: S_HeaderDataTarget = reactive({ tipologiaOggetto: '', soggetto: '', datazione: '', istitutoDiConservazione: '' })
const metaData: S_MetaDataTarget = reactive({ titolo: '', autore: '', soggetto: '', datazione: '', editori: '', tiratura: '', iscrizioni: '', stemmi: '', tipologiaOggetto: '', funzionaOriginaria: '', tecnica_e_supporto: '', misure: '', acquisizione: '', analyticalTechnique: '', istitutoDiConservazione: '', inventarioNumero: '', analisiDelBene: '', mostre: '', bibliografia: '', bibliografia_AD: '', statoDiConservazione: '', restauri: '', analisiDiagnostiche: '', bibliografiaRelativaRestauro: '', corniceBasamentoSupporto: '' })
const aboutTheWorkData: S_AboutTheWorkDataTarget = reactive({ about: '' })
const manifestData: S_ManifestDataTarget = reactive({ autore: '', anno: '', tecnica: '', luogo: '' })
const campiAggiuntiviData: CampiAggiuntiviDataTarget = reactive({ campoAggiuntivo: [{ label: '', value: '' }] })

const titleDataExtractor = new S_TitleData(titleData)
const headerDataExtractor = new S_HeaderData(headerData)
const aboutTheWorkExtractor = new S_AboutTheWorkData(aboutTheWorkData)
const metaDataExtractor = new S_MetaData(metaData)
const manifestExtractor = new S_ManifestData(manifestData)
