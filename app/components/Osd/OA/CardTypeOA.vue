...
import type { OA_AboutTheWorkDataTarget } from '~/utils/AboutTheWorkData'
import type { CampiAggiuntiviDataTarget } from '~/utils/CampiAggiuntivi'
import type { OA_HeaderDataTarget } from '~/utils/HeaderData'
import type { OA_ManifestDataTarget } from '~/utils/ManifestData'
import type { OA_MetaDataTarget } from '~/utils/MetaData'
import type { OA_TitleDataTarget } from '~/utils/TitleData'
...
import { OA_AboutTheWorkData } from '~/utils/AboutTheWorkData'
import { GEN_CampiAggiuntiviData } from '~/utils/CampiAggiuntivi'
import { OA_HeaderData } from '~/utils/HeaderData'
import { OA_ManifestData } from '~/utils/ManifestData'
import { OA_MetaData } from '~/utils/MetaData'
import { OA_TitleData } from '~/utils/TitleData'
...
const titleData: OA_TitleDataTarget = reactive({ autore: '', titolo: '', anno: '' })
const headerData: OA_HeaderDataTarget = reactive({ tipologiaOggetto: '', istitutoDiConservazione: '', soggetto: '', datazione: '' })
const metaData: OA_MetaDataTarget = reactive({
  titolo: '',
  autore: '',
  soggetto: '',
  datazione: '',
  ...
  restauri: '',
  analisiDiagnostiche: '',
  bibliografiaRelativaRestauro: '',
  corniceBasamentoSupporto: '',
})
const aboutTheWorkData: OA_AboutTheWorkDataTarget = reactive({ about: '' })
const manifestData: OA_ManifestDataTarget = reactive({ autore: '', anno: '', tecnica: '', luogo: '' })
const campiAggiuntiviData: CampiAggiuntiviDataTarget = reactive({ campoAggiuntivo: [{ label: '', value: '' }] })

const titleDataExtractor = new OA_TitleData(titleData)
const headerDataExtractor = new OA_HeaderData(headerData)
const aboutTheWorkExtractor = new OA_AboutTheWorkData(aboutTheWorkData)
const metaDataExtractor = new OA_MetaData(metaData)
const manifestExtractor = new OA_ManifestData(manifestData)
