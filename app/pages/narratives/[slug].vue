<template>
  <ClientOnly>
    <div class="narrative-page" :class="{ 'narrative-page--css-motion': cssMotionEnabled }">
...
import { isStoryDevCssMotionEnabled } from '#shared/constants/storyDevMotion'
...
import StoryBibliographySection from '~/components/Narratives/sections/StoryBibliographySection.vue'
import StoryCatalogoOpereCitateSection from '~/components/Narratives/sections/StoryCatalogoOpereCitateSection.vue'
import StoryCreditsSection from '~/components/Narratives/sections/StoryCreditsSection.vue'
import StorySitographySection from '~/components/Narratives/sections/StorySitographySection.vue'
import StoryFullscreenStack from '~/components/Narratives/StoryFullscreenStack.vue'
import StoryHeader from '~/components/Narratives/StoryHeader.vue'
...
import '~/assets/styles/narratives-dev-motion.css'
...
const cssMotionEnabled = isStoryDevCssMotionEnabled()
...
let narrativeLayoutRefreshTimer: ReturnType<typeof setTimeout> | null = null

function onNarrativeWindowResize() {
  if (narrativeLayoutRefreshTimer)
    clearTimeout(narrativeLayoutRefreshTimer)
  narrativeLayoutRefreshTimer = setTimeout(() => {
    narrativeLayoutRefreshTimer = null
    requestStoryScrollLayoutRefresh()
  }, 150)
}

onMounted(() => {
  window.addEventListener('resize', onNarrativeWindowResize, { passive: true })
})

onUnmounted(() => {
  if (narrativeLayoutRefreshTimer)
    clearTimeout(narrativeLayoutRefreshTimer)
  window.removeEventListener('resize', onNarrativeWindowResize)
})
</script>
